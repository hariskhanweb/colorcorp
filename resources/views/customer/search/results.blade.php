...
<section class="lg:py-14 md:py-10 py-8" data-animation="slideInUp" data-animation-delay=".2s">
    <div class="max-w-screen-lg mx-auto">
        ...
        <div class="jm_product_outer_pg flex flex-wrap lg:pb-10 md:pb-8 pb-6">
            ...
            @php 
              $fimage = App\Helpers\Helpers::getFeaturedImage($prodlist->id);
              if(isset($prodlist->category_id)) {
                $catslug = App\Helpers\Helpers::getCategorySlug($prodlist->category_id);
              } else { 
                $catslug = App\Helpers\Helpers::getCategorySlug($prodlist->parent_category_id);
              }
            @endphp
            <div class="jm_product_single xl:w-1/4 lg:w-1/3 md:w-1/2 w-full mb-5 text-center">
                <div class="jm_product_outer product_card mx-3">
                    <a href="{{ route('product.single', ['vendor_name' => $shopslug,'category' => $catslug,'prodslug'=>$prodlist->slug]) }}" class="border-b border-gray-200 border-solid inline-block w-full">
                    ...
                    <div class="porduct_card_content lg:py-8 lg:px-6 md:py-4 md:px-4 p-8">
                        <a href="{{ route('product.single', ['vendor_name' => $shopslug,'category' => $catslug,'prodslug'=>$prodlist->slug]) }}">
                    ...
                    <a href="{{ route('product.single', ['vendor_name' => $shopslug,'category' => $catslug,'prodslug'=>$prodlist->slug]) }}" class="block uppercase text-sm green_btn lg:px-10 md:px-4"><span>{{ __('Detail') }} </span></a>
                </div>
            </div>
            @endforeach
            <!-- Product List Ends -->
